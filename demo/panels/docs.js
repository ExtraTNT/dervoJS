...
export const debugPanel = state =>
  ...
  {
    name: 'createCrud',
    description: 'Curried factory that turns an OpenAPI 3.0 or 3.1 spec into ready-to-render CRUD views (list / show / new / edit / delete). Each curry step gives a reusable handle: createCrud(http)(basePath)(spec)(resource)(opts). Form components and validators come from the schema automatically — TextInput for strings, NumberInput for numbers, Select for enums, Toggle for booleans, DateTimePicker for date/date-time, nested Cards for objects, repeater for arrays.',
    ...
    props: [
      ...
      { name: 'opts.id',           type: 'string | number',              default: 'state.id', desc: 'Override state.id (for show/edit/delete URLs).' },
      { name: 'opts.hideFields',   type: 'string[]',                     default: '[]',    desc: 'Field names to hide from every view.' },
      { name: 'opts.listColumns',  type: 'string[]',                     default: '— (auto)', desc: 'Pin which fields appear as list columns. Default: every scalar field.' },
      { name: 'opts.mountId',      type: 'string',                       default: "'crud-<resource>'", desc: 'Prefix used for input ids when multiple CrudResources render on the same page.' },
      { name: 'opts.onSuccess',    type: '(data) => void',               default: '—',     desc: 'Called after a successful create / update / delete.' },
      { name: 'opts.onError',      type: '(err) => void',                default: '—',     desc: 'Called when a mutation fails.' },
    ],
  },
  {
    name: 'createHttp',
  ...
  },
  {
    ...
    description: 'Pure compiler: turns an OpenAPI document into the descriptors CrudResource consumes. Useful for debugging or for building your own renderers on top of the same schema info. Operation discovery is path-based — "users" matches /users, /api/users, /v1/users/{id}.',
    example: "import { compileResource } from './src/index.js';\nconst c = compileResource(spec)('users');\n// { ops, idParam, fields, showFields, listItemFields }\nconsole.log(c.ops.listOp?.method);   // 'get'\nconsole.log(c.fields.map(f => f.name));",
    props: [

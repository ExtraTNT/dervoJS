...
const _BuilderTab = () => div({})([
  ...
  _section('Rooms tab',
    ...
    _row('Auto-NPCs',     [
      'Any NPC at this room gets an auto greeting line + "Talk to <name>" choice. No wiring needed. The choice label can be overridden per-NPC via ',
      _kbd('interactLabel'), ' on the NPC config - set it to ', _kbd('Use workbench'), ' / ', _kbd('Read tome'),
      ' / ', _kbd('Pray at altar'), ' when the NPC isn\'t a person.',
    ]),
  ),
  ...
  _section('Story Points tab',
    ...
    _row('Exits',       ['NOTHING is created by default. You opt in to either: (a) explicit ', _kbd('Choices'),
      ' - same editor as scene rooms; targets include every room incl. other story points (those get a ', span({ className: 'dv-mono' })(['⭐']),
  ...
  _section('Effect modes (any Effect field - Choice action, room onEnter / onEnd, item useEffect, combat onWin / onLose, NPC topic onEnter)',
    ...
    _row('navigate',     ['Pick a target room (any kind, including story points marked ', span({ className: 'dv-mono' })(['⭐']),
  ...
  _section('NPCs tab',
    ...
    _row('role: shop · buyback', [
      ...
      _kbd('floor(multiplier x item.price.amount)'),
  ...
  _section('Add component (topbar)',
    ...
    _row('🗺️ Map Builder',[
      'Graphical room layout. Drop room cards on a canvas, drag them around, connect with wires (',
      _kbd('→'), ' starts a wire from a room, click another room to complete it). Each room only needs a name - the wizard\'s quick-build promise. On Finish every node becomes a real scene room in folder ',
      _kbd('<prefix>/map'), '; existing rooms stay untouched (additive).',
    ]),
    _row('🗺️ Map Builder - wire direction', [
      'Wires are ', _kbd('↔ bidirectional'),
      ' by default - solid line, no arrowhead. The build emits a Choice on BOTH endpoints (',
      _kbd('A → "Go to B"'), ' on A, ', _kbd('B → "Go to A"'), ' on B). Click the ',
      _kbd('↔'), ' mid-wire button to flip to ', _kbd('→ one-way'),
      ' - dashed line + arrowhead toward the target, and only the source room gets the Choice (one-way gate). A toolbar toggle pre-picks the direction for newly drawn wires; per-wire toggles override it after the fact.',
    ]),
    _row('📜 Quest Giver',[
    ...
    _row('⚒️ Crafting workbench',[
      'Workbench NPC dropped into a chosen scene room (existing or freshly created). NPC is ',
      _kbd('advanced'), ' mode: a top-level ', _kbd('Menu'),
      ' topic fans out into one sub-topic per recipe (', _kbd('flow: change'),
      '), and Goodbye pops back out (', _kbd('flow: exitBack'),
      '). Each recipe sub-topic has a Craft choice gated by a JS ',
      _kbd('hasItem ALL'),
      ' condition (ingredients x qty), an action that subtracts every ingredient and adds the output (one ',
      _kbd('inv.<id>'), ' op each), and ', _kbd('flow: exitBack'),
      ' back to the Menu so the player can keep crafting. Items must already exist on the Items tab - the builder won\'t auto-create them (avoids polluting the catalogue with placeholder ids).',
    ]),
    _row('🍺 Tavern + Healer',[
    ...
    _row('👥 Random NPCs',[
      'Bulk-cast a town from grouped pools. ', _kbd('Groups'),
      ' are tags (male / female / order / guard / …). Each name in the ', _kbd('Names'),
      ' pool carries an optional group suffix: ', _kbd('Aldo: male'), ', ', _kbd('Riley: male, female'),
      ' (unisex), or bare ', _kbd('Aldo'), ' (no tags). All four other pools - ',
      _kbd('portraits'), ', ', _kbd('conversation topics'), ', and ', _kbd('quests'),
      ' - declare which groups they require. An entry is eligible for an NPC only when its required tags are a ',
      span({ style: 'font-weight:600' })(['subset']), ' of the NPC\'s tags. A "tampons" quest tagged ',
      _kbd('female'), ' will never land on a male-only NPC; an "armoured guard" portrait tagged ',
      _kbd('male, guard'), ' only attaches to NPCs that carry both tags; an "army talk" topic tagged ',
      _kbd('male'), ' just won\'t appear on a commoner woman.',
    ]),
    _row('👥 Random NPCs - quest pipeline', [
      'Each assigned quest gets the ', _kbd('questGiver'), ' shape - same shared ', _kbd('_quests.js'),
      ' helpers used by the dedicated builder. Per-NPC trio: ', _kbd('offer'), ' (Accept sets ',
      _kbd('q_<id>_started'), '), ', _kbd('progress'), ' (hint page), ',
      _kbd('turn-in'), ' (', _kbd('onEnter'), ' fires extra ops then sets ', _kbd('q_<id>_done'),
      '). The same NPC owns offer → progress → turn-in end to end. The shared ',
      _kbd('quest_log'), ' room + ', _kbd('📜 Quests'),
      ' sidebar link get added the first time and reused on every subsequent run (questGiver-built and randomNpcs-built quests share the same log).',
    ]),
    _row('👥 Random NPCs - goal types', [
      'Each quest picks a goal. ',
      _kbd('flag'), ' (recommended for one-shots): pick an existing flag or create a new one (e.g. ',
      _kbd('tower_gate_up'), ') + expected value. The player triggers the flag elsewhere (lever, room onEnter, combat onWin) and the quest auto-completes. ',
      _kbd('fetch'), ': fixed item + count, or random pool + range rolled per copy. Turn-in consumes the items via an ',
      _kbd('inv.<id> take N'), ' op. ',
      _kbd('fight'), ': pick an existing combat (builder injects ', _kbd('flags.combat_<id>_won = true'),
      ' on its onWin - idempotent, wraps non-simple onWin in ', _kbd('multi'),
      ') OR have the wizard create a fresh combat per copy with rolled stats. Completion expression is computed via the shared ',
      _kbd('goalCompletionExpr'), ', so the player only sees the turn-in option when the goal is met.',
    ]),
    _row('👥 Random NPCs - quest text + placeholders', [
      'Quest title / offer / progressHint / done are templates that interpolate the NPC scope (',
      _kbd('${name}'), ', ', _kbd('${groups}'), ', ', _kbd('${index}'),
      ') AND a goal-specific scope: ', _kbd('${item}'), ' + ', _kbd('${count}'), ' + ', _kbd('${itemId}'),
      ' for ', _kbd('fetch'), '; ', _kbd('${enemy}'), ' (+ ', _kbd('${hp}'), ' / ', _kbd('${damage}'),
      ' for fight-create) for ', _kbd('fight'), '; ', _kbd('${flag}'), ' for ', _kbd('flag'),
      '. Fetch quests AUTO-FILL when offer / progress / done are blank ("Bring me ${count} ${item}.", "Still need those ${item}.", "Thanks for the ${item}."). Flag and fight take custom messages (no obvious default) - the validator enforces them.',
    ]),
    _row('👥 Random NPCs - templating', [
      'Topic responses + quest title / offer / progressHint / done text interpolate ', _kbd('${name}'), ', ', _kbd('${groups}'),
      ' (array), and ', _kbd('${index}'), ' (generation index) at BUILD time - generated topics get static text baked in (one template, per-NPC realisations). JS expressions inside ',
      _kbd('${…}'), ' work: ', _kbd('${groups.includes("order") ? "I serve the Order." : "I trade beans."}'),
      '. Template failures swallow to empty string so a typo in one expression doesn\'t poison the whole page.',
    ]),
    _row('👥 Random NPCs - distribution', [
      _kbd('Crowd %'), ' picks which NPCs WANDER - each gets ', _kbd('wanderRooms'), ' locations and ',
      _kbd('ctx.tickWorld()'), ' hops them between rooms. ',
      _kbd('Absent %'), ' lands NPCs with ', _kbd('locations: []'), ' (exist but never spawn). ',
      _kbd('Wander off-area %'), ' (crowd only) sprinkles empty-string slots into the locations array - ',
      _kbd('npcsAt'), ' never matches an empty sceneId, so a wanderer occasionally goes "off the playable map" for a tick.',
    ]),
    _row('👥 Random NPCs - mass-gen knobs', [
      'Quests are ', span({ style: 'font-weight:600' })(['templates']),
      ' - each spawns ', _kbd('copies'),
      ' concrete quests with per-copy randomisation. ',
      _kbd('fetch random'), ' rolls one item from a checkbox pool + a count from a (min, max) range per copy. ',
      _kbd('fight create'), ' creates a fresh combat per copy with rolled HP / damage stats; the enemy name template gets the giver\'s ',
      _kbd('${name}'), ' in scope. ',
      _kbd('flag'), ' goals (when ', _kbd('flagPerCopy'),
      ' is on) suffix ', _kbd('_<i>'), ' on the flag name so 20 copies of "lost_locket" become "lost_locket_1" … "lost_locket_20" - each NPC works on a distinct flag (turn off for shared world-event flags).',
    ]),
    _row('👥 Random NPCs - topic guards + rewards', [
      'Each topic carries an optional ', _kbd('guard'),
      ' that gates the menu choice at runtime - ', _kbd('flag'),
      ' (e.g. ', _kbd('tower_gate_down == true'), ' → "About the gate" only shows up after the gate drops) or ',
      _kbd('stat'), ' (e.g. ', _kbd('hp ≤ 7'), ' → "Concerned look", ', _kbd('str ≥ 10'), ' → "Impressed"). Each topic also has an optional once-only ',
      _kbd('reward'), '. The amount is rolled per NPC in [min, max] at gen time (20 NPCs → 20 different fixed tips). On the player\'s Goodbye click, a two-op simple effect adds the reward stat and flips a ',
      _kbd('<topicId>_claimed'), ' flag, both gated by ', _kbd('!claimed'),
      ' so a second visit pays nothing. The claimed flag is per topic-instance (NOT per template) so each NPC tracks their own payout.',
    ]),
    _row('👥 Random NPCs - names + folder', [
      _kbd('Name prefix / suffix'), ' templates wrap the rolled base name from the pool. Both see ',
      _kbd('${name}'), ' (base), ', _kbd('${groups}'),
      ' (tag array) and ', _kbd('${index}'), '. Conditional examples: ',
      _kbd('${groups.includes("guard") ? " the Guard" : ""}'),
      ' (suffix), ', _kbd('${groups.includes("priest") ? "Father " : ""}'),
      ' (prefix). The composed full name is what subsequent topic / quest templates see in their ',
      _kbd('${name}'), '. ', _kbd('Folder prefix'), ' lands every generated NPC inside ',
      _kbd('<prefix>/<distribution>[/<groups>]'),
      ' and back-tags any catalogue assets used as portraits when their folder is empty - one click to clean up a 200-NPC batch.',
    ]),
    ...
    _row('Adding more',  ['New builders go in ', _kbd('gameEditor/components/builders/<id>.js'), ' exporting ', _kbd('{ id, icon, name, description, defaults, steps, build }'),
      ' and get listed in ', _kbd('ComponentBuilder.js'), '. Use the shared helpers in ', _kbd('helpers.js'), ' (', _kbd('uniqueId'), ', ',
  ...
  _section('Sidebar tab',
    ...
    _row('Widget types',   ['title / portrait / stats / inventory / roomLink (button to a room) / minimap / js (custom render).']),
    _row('Minimap',        [
      'Auto-discovers reachable rooms via BFS over choice exits - no room list to maintain. ',
      _kbd('kind: story'), ' rooms and any room with ', _kbd('Hide from Graph + minimap'),
      ' ticked are filtered BEFORE the BFS, so they never appear and can\'t act as a stepping-stone between two visible rooms. ',
      'Tracks visited state in ', _kbd('state._mapVisited'),
      ' (the widget itself does the write, microtask-deferred so it doesn\'t mutate mid-render). Current scene fills with ',
      _kbd('var(--accent)'), '; visited rooms get a muted fill; unvisited rooms are outline-only when ', _kbd('Dim unvisited'),
      ' is on. Edges between rooms are deduped (bidir pair counted once).',
    ]),
    _row('Minimap - layout', [
      'Grid coords are computed ONCE per project (preview recomputes per render; codegen precomputes at export time and embeds ',
      _kbd('MINIMAP_ROOMS'), '). BFS from ', _kbd('meta.start'),
      '; each newly-discovered room is placed at the cell pointed at by its incoming choice\'s direction word - whole-word match on ',
      _kbd('north / up / above'), ', ', _kbd('south / down / below'), ', ', _kbd('east / right'), ', ', _kbd('west / left'),
      '. If the hinted cell is taken, fall back to the four cardinals; if all four are taken, spiral outward. ',
      'Layout never depends on visited / fog / range, so revealing a new room never shifts the others. Edges between rooms in the same row or column draw as straight lines; mismatched coords use an L-shaped polyline - never diagonal.',
    ]),
    _row('Minimap - knobs', [
      _kbd('width'), ' rendered SVG width in CSS px (0 = natural size; viewBox preserves the aspect when scaled) · ',
      _kbd('zoom'), ' max viewport size in CSS px - wraps the SVG in an ', _kbd('overflow:auto'),
      ' div capped at that size on both axes so the player can pan around a larger map (', _kbd('0'), ' = off; SVG fits the container as before) · ',
      _kbd('range'), ' BFS depth from the player\'s current room (',
      _kbd('0'), ' = static full map BFS\'d from ', _kbd('meta.start'), '; ', _kbd('1+'),
      ' re-roots the BFS at the player every frame, so the rendered subset follows them) · ',
      _kbd('visitedOnly'), ' fog-of-war: only render rooms the player has visited (or the room they\'re in right now) · ',
      _kbd('reach'), ' when fog is on, expand the visible set by N BFS steps from any visited room so the player sees adjacent unexplored rooms as outlines (', _kbd('0'), ' = strict - only visited cells appear) · ',
      _kbd('fastTravel'), ' click any visible cell → ', _kbd('ctx.goto(target)'),
      ' if a directional path exists through visited rooms with passing conditions (target also has to be visited; one-way edges are honoured - if only ', _kbd('B → A'), ' is wired, clicking B from A does nothing) · ',
      _kbd('showLabels'), ' room titles inside cells (bigger cells) · ', _kbd('label'),
      ' heading text (empty hides the heading). Combat / NPC dialogue: highlight + range + visited write all center on ',
      _kbd('state._combat.returnTo'), ' / the calling room so the map keeps tracking the player\'s physical location.',
    ]),
    _row('Sidebar width', [
      _kbd('project.sidebar.width'), ' is a CSS string (e.g. ', _kbd('320px'), ', ', _kbd('20rem'), ', ', _kbd('min(30vw, 380px)'),
      '). Applied as ', _kbd('--sidebar-width'),
      ' on :root at game-mount time, overriding the engine\'s ', _kbd('240px'), ' default in dervo.css. Empty = use the default. Preview scopes the override to its overlay so the editor\'s own sidebar (hidden behind it) isn\'t affected.',
    ]),
  ),

  _section('Project tab',
    _row('Stats',          [
      'Values seeded into ', _kbd('state.<key>'), '. Type drives the ops surface: ',
      _kbd('number'), ' (default) gets ', _kbd('add / sub / set'), ' + clamps and ',
      _kbd('>=, >, <=, <, ==, !='), ' conditions; ',
      _kbd('string'), ' gets ', _kbd('set / append / clear'), ' ops and ',
      _kbd('==, !=, contains, startsWith, isEmpty'), ' conditions; ',
      _kbd('array'), ' gets ', _kbd('push / removeValue / clear / set'), ' ops and ',
      _kbd('includes, excludes, lenAtLeast, isEmpty'),
      ' conditions. Engine has always accepted non-numeric state values - only the editor surface used to limit them to numbers. Pick-list labels include the type suffix (e.g. ',
      _kbd('stat: gender (string)'),
      ') so author can tell them apart. Array editing in the Project tab takes comma-separated input as a convenience and stores ',
      _kbd('Array<string>'), '.',
    ]),
    _row('Numeric-only pickers', [
      'Places that need arithmetic (item price stat, skill damage / cost / hit stats, combat ',
      _kbd('playerStat'), ', op clamp formulas ', _kbd('mul * state[k] + const'),
      ', weight bonuses, loot table costs, builder defaults) filter their dropdowns to ',
      _kbd('type === \'number\''), ' so the author can\'t accidentally point them at a string / array stat. Effect / Condition targets stay all-stats - those are the places where typed ops actually matter.',
    ]),
  ...
  _section('Character creation tab',
    _row('What it is',     [
      'A pre-game wizard. When ', _kbd('Show character creation at game start'),
      ' is on AND at least one step exists, the engine renders the steps in order BEFORE the player ever sees ',
      _kbd('meta.start'), '. The wizard is skipped entirely (no scene swap, no state write) when disabled OR when there are no steps.',
    ]),
    _row('Runtime state',  [
      _kbd('state._cc = { step, picks, points, startRoom, done }'),
      '. ', _kbd('picks: { stepId: optionId }'), ' for choice steps; ',
      _kbd('points: { stepId: { statKey: value } }'), ' for pointBuy. ', _kbd('startRoom'),
      ' tracks the last non-empty option override; the final step\'s advance navigates there (or ',
      _kbd('meta.start'), ' if nothing wins).',
    ]),
    _row('Step · Point-buy',[
      'Distribute a fixed ', _kbd('budget'), ' across selected numeric stats. Each row carries ',
      _kbd('{ statKey, min, max, start }'), '. The player can spend remaining points by raising stats (capped by ', _kbd('max'),
      ') or recoup by lowering (down to ', _kbd('min'),
      '). Continue is gated by ', _kbd('remaining >= 0'),
      '. On advance, each stat is patched to its allocated value (', _kbd('state.<key>'), ').',
    ]),
    _row('Step · Choice', [
      'Player picks ONE option. Each option carries ', _kbd('{ label, description, image, effect, startRoom }'),
      '. The Effect is any normal Effect (set stat, give item, set flag, run js, …). ', _kbd('startRoom'),
      ' optionally overrides ', _kbd('meta.start'), ' - the LAST step\'s pick with a non-empty startRoom wins, so a background-choice step can route the player to a different opening room than a class-choice step that runs later.',
    ]),
    _row('NPC reactions', [
      'Any text field in any panel goes through ', _kbd('${…}'),
      ' JS interpolation at render time with state vars in scope. So a NPC greeting like ',
      _kbd('Hello ${gender === "male" ? "sir" : "ma\'am"}'), ' Just Works - and the wizard\'s string-typed ',
      _kbd('gender'), ' stat is the source. No special NPC config needed for light reactions.',
    ]),
    _row('NPC variants', [
      'For deeper reactions, an NPC carries an optional ', _kbd('variants'), ' list. Each variant has a ',
      _kbd('condition'), ' and an ', _kbd('overrides'),
      ' partial. At render time, the engine scans variants in order; the first matching condition\'s overrides are merged onto the base NPC (greeting, portrait, pages, choices, topics, entryTopicId). Empty / blank override fields fall through to base. Codegen precompiles each variant\'s dialogue so the runtime cost is one guard check per variant - no JSON re-parse, no dynamic eval.',
    ]),
    _row('Skipped on load', [
      'The wizard runs on a fresh start. Loaded saves keep their state - including ',
      _kbd('_cc.done = true'), ' - so the engine routes straight into the resolved start room.',
    ]),
  ),
  ...
  _section('Graph tab',
    ...
    _row('Hidden rooms',   [
      'Rooms with ', _kbd('kind: story'), ' are always excluded (narrative-only, no choice exits to walk through). ',
      'Authors can also tick ', _kbd('Hide from Graph + minimap'),
      ' on any other room - useful for hub / dream / void rooms whose physical layout would just clutter the map. The flag is the same one the in-game minimap reads, so both views stay consistent.',
    ]),
  ),
]);
...
c.setState(s => ({
  ...
  _section('Reading (Condition + JS)',
    ...
    _row('random condition', ['Slider 0–1. Compiles to ', _kbd('Math.random() < p'), '. Use for "wild encounter 30% of the time" or any RNG gate.']),
...
const _StateTab = () => div({})([
  ...
  _code([
    ...
    'state._mapVisited       // { roomId: true } - minimap visited set (widget-driven)',
    'state._cc               // character-creation progress: { step, picks, points, startRoom, done } (only when project.charCreation.enabled)',
    '',
    '// NPC world',

...
const _initialStateShape = project => {
  const stats = Object.fromEntries((project.stats || []).map(s => [s.key, Number(s.initial) || 0]));
...
const _intro = () => Stack({ gap: 8 })([
  ...
  p({ style: 'margin:0; font-size:11.5px; color:var(--text-muted)' })([
    'Expressions run via ', span({ style: 'font-family:ui-monospace,monospace' })(['with (state)']),
    ' so bare ', span({ style: 'font-family:ui-monospace,monospace' })(['gold']),
    ' resolves to ', span({ style: 'font-family:ui-monospace,monospace' })(['state.gold']),
    '. Compile errors leave the source ', span({ style: 'font-family:ui-monospace,monospace' })(['${expr}']),

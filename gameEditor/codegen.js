...
const emitMain = project => {
  ...
  const defaultMusicLit = _q(project.meta?.defaultMusic || '');
  const musicLine = musicByRoom || defaultMusicLit !== '""'
    ? `\nconst MUSIC = {\n${musicByRoom}\n};\nconst DEFAULT_MUSIC = ${defaultMusicLit};\nconst music = ctx => {\n  let id = ctx.scene;\n  if (typeof id === 'string' && id.startsWith('_combat:')) id = ctx.state._combat?.returnTo || id;\n  return MUSIC[id] || DEFAULT_MUSIC;\n};\n`
    : '';
  const musicArg = musicLine ? '  music,\n' : '';
